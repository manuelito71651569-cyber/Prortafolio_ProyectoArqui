...
const VISOR = {
  ...
  open(archivo, uploadsBasePath) {
    ...
    var ext  = getExtension(archivo.ruta || archivo.nombre || archivo.archivo || '');
    ...
    var path = archivo.dataUrl
      || archivo.objectUrl
      || (archivo.ruta && archivo.ruta.startsWith('http') ? archivo.ruta : (base + (archivo.ruta || '')));
  ...
  _renderBody(ext, path, archivo) {
    ...
    if (isImage(ext)) {
      ...
      // ── VIDEO ──
      body.innerHTML =
        '<video controls style="max-width:100%;max-height:65vh;width:100%;background:#000" src="' + path + '"></video>';

    } else if (['mp3','wav'].includes(ext)) {
      // ── AUDIO ──
      body.innerHTML =
        '<div style="padding:40px;text-align:center"><div style="font-size:4rem;margin-bottom:20px">🎵</div>' +
        '<audio controls style="width:100%" src="' + path + '"></audio></div>';
