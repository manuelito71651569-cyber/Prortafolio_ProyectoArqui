...
const ADMIN = {
  ...
  async cargarDatosPerfil() {
    var p = await window.SupabaseCtrl.getPerfil();
    if (!p) { TOAST.show('Error al cargar perfil', 'error'); return; }
  ...
  async previsualizarRuta() {
    ...
    if (!prev) return;
    if (cursoId && archivo) {
      prev.textContent = 'assets/uploads/cursos/' + cursoId + '/semana-' + this._semanaSeleccionada + '/' + archivo;
    } else if (cursoId) {
      prev.textContent = 'assets/uploads/cursos/' + cursoId + '/semana-' + this._semanaSeleccionada + '/[archivo]';
    } else {
  ...
  async registrarArchivo() {
    ...
    var cursoId  = elCurso  ? parseInt(elCurso.value.trim())  : null;
    ...
    try {
      ...
      var publicUrl = await window.SupabaseCtrl.uploadStorageFile(file);
      if (!publicUrl) throw new Error("Fallo al subir a Storage");
      ...
      var ok = await window.SupabaseCtrl.createArchivo(cursoId, semana, datosArchivo);
      if (ok) {
        ...
        setTimeout(function() { switchCrudTab('arc-lista'); }, 700);
      } else {
    ...
    cursos.forEach(function(c) {
      c.semanas.forEach(function(s) {
        ...
        s.archivos.forEach(function(a) {
          ...
          rows.push({ curso:c, semana:s, archivo:a, ext: getExtension(a.ruta) || getExtension(a.nombre) });
        });
      });
    });
      ...
      html +=
        '<tr>' +
          ...
          '<td>' +
            ...
            (a.created_at ? '<div style="font-size:.68rem;color:var(--txt-m);margin-top:2px">📅 ' + new Date(a.created_at).toLocaleDateString() + '</div>' : '') +
            (estadoBadge ? '<div>' + estadoBadge + '</div>' : '') +
          '</td>' +
          '<td><span style="font-family:monospace;font-size:.74rem;color:var(--txt-m)">' + (a.ruta ? 'Archivo Guardado' : '') + '</span> <span class="ext-badge">' + r.ext + '</span></td>' +
  ...
  async eliminarArchivo(cursoId, semanaNum, archivoId) {
    await window.SupabaseCtrl.deleteArchivo(archivoId);
